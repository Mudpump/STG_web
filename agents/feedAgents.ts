...
export async function runAgentA(
  ...
  // 2. Select Subject based on Grade Mapping
  const gradeKey = (targetGrade === 'ALL' || targetGrade === 'MIDDLE') ? 'H1' : targetGrade;
  
  const subjectsList = GRADE_SUBJECT_MAPPING[targetCategoryId]?.[gradeKey] || GRADE_SUBJECT_MAPPING[targetCategoryId]?.['H1'] || ['진로와 직업'];
  ...
  if (monthlyData) {
      ...
      if (matchingKey) {
          curriculumContext = `
            ...
            - Instruction: The student is currently learning this specific unit. Connect the topic DIRECTLY to this unit's concepts.
          `;
      } else {
  ...
  log(`🎯 타겟: [${targetCategoryName}] - [${gradeKey} / ${targetMonth}월] - [${selectedSubject}] - [전략: ${selectedLogicKey}]`);
...
export async function runAgentB(topicData: any, author: Agent, commenters: Agent[], log: (msg: string) => void, feedback?: string) {
  ...
  const taskInstruction = `
    ...
    2. Write 4~6 Comments using the [Commenters]. 
       ...
       - Since they are students, explain **easy ways to plan experiments** in the initial stage and provide **realistic methods** to concretize the experiment.
